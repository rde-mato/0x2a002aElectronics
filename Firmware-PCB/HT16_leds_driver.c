...
inline void	led_set(u8 index)
{
	all.current_leds_on |= (1 << index);
	HT16_write_leds_request = 1;
}

inline void	led_toggle(u8 index)
{
	all.current_leds_on ^= (1 << index);
	HT16_write_leds_request = 1;
}

inline void	led_clr(u8 index)
{
	all.current_leds_on &= ~(1 << index);
	HT16_write_leds_request = 1;
}

inline void	clear_led_request(void)
{
